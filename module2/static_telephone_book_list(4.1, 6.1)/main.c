...
static void show_all(contact_book *book)
{
	struct contact sorted[TABLE_SIZE];
	unsigned int n;
	unsigned int i;

	n = contact_sorted_copy(book, sorted);
	if (n == 0) {
		printf("Список пуст.\n");
		return;
	}
...
static void action_add(contact_book *book)
{
	struct contact c;
...
static void action_edit(contact_book *book, unsigned int id)
{
	struct contact *c;
	int field;

	c = contact_get(book, id);
	if (!c) {
		printf("Контакт не найден.\n");
		return;
	}

	printf("1-фамилия 2-имя 3-отчество 4-работа 5-должность\n");
	printf("6-телефон 7-email 8-соцсеть 9-мессенджер 0-отмена\n");
	printf("Поле: ");
	if (scanf("%d", &field) != 1)
		return;
	flush_line();
...
static void action_find(contact_book *book)
{
	...
	n = contact_search(book, surname, name, found, TABLE_SIZE);
	if (n == 0) {
		printf("Не найден.\n");
		return;
	}
...
static void action_delete(contact_book *book, unsigned int id)
{
	if (contact_delete(book, id) < 0)
		printf("Контакт не найден.\n");
	else
...
int main(void)
{
	...
	book = contact_book_create();
	if (!book) {
		printf("Не удалось создать телефонную книгу.\n");
		return 1;
	}
	...
	for (;;) {
		...
		case 5:
			...
			if (scanf("%u", &id) == 1) {
				flush_line();
				action_delete(book, id);
			}
			break;
		case 6:
			if (contact_save(book) < 0)
				printf("Ошибка сохранения.\n");
			else
				printf("Сохранено.\n");
			break;
		case 7:
			if (contact_load(book) < 0)
				printf("Ошибка загрузки.\n");
			else
				printf("Загружено: %u\n", contact_book_count(book));
			break;
		case 0:
